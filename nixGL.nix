...
let
  ...
  writeNixGL =
    ...
    writeExecutable {
      ...
      text =
        let
          mesa-drivers = [ mesa ] ++ lib.optional enable32bits pkgsi686Linux.mesa;
        ...
        ''
          ...
          ${
            ''export __EGL_VENDOR_LIBRARY_FILENAMES=${mesa}/share/glvnd/egl_vendor.d/50_mesa.json${lib.optionalString enable32bits ":${pkgsi686Linux.mesa}/share/glvnd/egl_vendor.d/50_mesa.json"}"''${__EGL_VENDOR_LIBRARY_FILENAMES:+:$__EGL_VENDOR_LIBRARY_FILENAMES}"''
          }
    ...
    nvidiaPackages =
      ...
      rec {
        nvidiaDrivers = (linuxPackages.nvidia_x11.override { }).overrideAttrs (oldAttrs: rec {
          pname = "nvidia";
          name = "nvidia-x11-${if version == null then "unknown" else version}-nixGL";
          inherit version;
          src =
            let
              url = "https://download.nvidia.com/XFree86/Linux-x86_64/${
                if version == null then "unknown" else version
              }/NVIDIA-Linux-x86_64-${if version == null then "unknown" else version}.run";
            in
        ...
        nixGLNvidiaBumblebee = writeExecutable {
          name = "nixGLNvidiaBumblebee-${if version == null then "unknown" else version}";
          text = ''
            #!${runtimeShell}
        ...
        nixNvidiaWrapper =
          api:
          writeExecutable {
            name = "nix${api}Nvidia-${if version == null then "unknown" else version}";
            text = ''
              #!${runtimeShell}
              ${lib.optionalString (
                api == "Vulkan"
